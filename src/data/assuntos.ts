import disciplinasJson from "./disciplinas_areas.json";

export type AreasMap = Record<string, { subareas: string[] }>;
export type NivelAreasMap = Partial<Record<"fundamental" | "medio" | "superior", AreasMap>>;

type DisciplinasJsonType = Record<string, NivelAreasMap>;
const DISCIPLINAS_JSON = disciplinasJson as DisciplinasJsonType;

// ======================== Mapa de áreas da Matemática (retrocompatibilidade) ========================

/**
 * Mapa de áreas unificado da Matemática (todos os níveis mesclados).
 * Mantido para compatibilidade com código legado.
 */
export const AREAS_MAP: AreasMap = (() => {
  const matEntry = DISCIPLINAS_JSON["Matemática"];
  if (!matEntry) return {};
  const merged: AreasMap = {};
  for (const n of ["fundamental", "medio", "superior"] as const) {
    const m = matEntry[n];
    if (m) Object.assign(merged, m);
  }
  return merged;
})();

// ======================== Mapa disciplina → NivelAreasMap ========================

// Chaves lowercase (com e sem acento) para lookup case-insensitive
const DISCIPLINAS_AREAS_MAP: Record<string, NivelAreasMap> = {
  // Cria entradas com acento E sem acento automaticamente
  ...Object.fromEntries(
    Object.entries(DISCIPLINAS_JSON).flatMap(([disc, nivelMap]) => [
      [disc.toLowerCase(), nivelMap],
      [disc.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""), nivelMap],
    ])
  ),
  // Aliases para nomes alternativos / abreviações
  "português":        DISCIPLINAS_JSON["Língua Portuguesa"]!,
  "portugues":        DISCIPLINAS_JSON["Língua Portuguesa"]!,
  "inglês":           DISCIPLINAS_JSON["Língua Inglesa"]!,
  "ingles":           DISCIPLINAS_JSON["Língua Inglesa"]!,
  "espanhol":         DISCIPLINAS_JSON["Língua Espanhola"]!,
  "francês":          DISCIPLINAS_JSON["Língua Francesa"]!,
  "frances":          DISCIPLINAS_JSON["Língua Francesa"]!,
  "arte":             DISCIPLINAS_JSON["Artes"]!,
  "ed. física":       DISCIPLINAS_JSON["Educação Física"]!,
  "ed. fisica":       DISCIPLINAS_JSON["Educação Física"]!,
};

/**
 * Retorna o AreasMap para uma disciplina e nível específico.
 *
 * - Se `nivel` informado e a disciplina tem conteúdo para esse nível: retorna exato.
 * - Se `nivel` informado mas sem conteúdo para ele: faz fallback para "medio".
 * - Se `nivel` não informado: mescla todos os níveis da disciplina.
 * - Retorna null se a disciplina não for encontrada.
 */
export function getAreasMapPorDisciplina(
  disciplina?: string | null,
  nivel?: string | null
): AreasMap | null {
  if (!disciplina || disciplina.trim() === "") return null;
  const comAcento = disciplina.trim().toLowerCase();
  const semAcento = comAcento.normalize("NFD").replace(/[\u0300-\u036f]/g, "");

  const nivelMap = DISCIPLINAS_AREAS_MAP[comAcento] ?? DISCIPLINAS_AREAS_MAP[semAcento] ?? null;
  if (!nivelMap) return null;

  if (nivel === "fundamental" || nivel === "medio" || nivel === "superior") {
    // Retorna o nível exato se disponível, senão fallback para "medio"
    return nivelMap[nivel] ?? nivelMap["medio"] ?? null;
  }

  // Sem nivel: mescla todos os níveis
  const merged: AreasMap = {};
  for (const n of ["fundamental", "medio", "superior"] as const) {
    const m = nivelMap[n];
    if (m) Object.assign(merged, m);
  }
  return Object.keys(merged).length > 0 ? merged : null;
}

/**
 * Retorna a lista de assuntos canônicos para uma disciplina (e opcionalmente nível).
 * Se disciplina não informada ou desconhecida, retorna TODOS os assuntos.
 */
export function getAssuntosPorDisciplina(
  disciplina?: string | null,
  nivel?: string | null
): string[] {
  const map = getAreasMapPorDisciplina(disciplina, nivel);
  if (!map) return ASSUNTOS_CANONICOS; // fallback: todos
  const set = new Set<string>();
  for (const [area, { subareas }] of Object.entries(map)) {
    set.add(area);
    for (const s of subareas) set.add(s);
  }
  return [...set].sort((a, b) => a.localeCompare(b, "pt-BR"));
}

// ======================== CANONICAL_NAMES ========================

// Mapa lowercase → grafia canônica de todos os nomes conhecidos (todas as disciplinas, todos os níveis)
const CANONICAL_NAMES: Record<string, string> = {};

for (const nivelMap of Object.values(DISCIPLINAS_JSON)) {
  for (const areasMap of Object.values(nivelMap)) {
    if (!areasMap) continue;
    for (const [area, { subareas }] of Object.entries(areasMap)) {
      CANONICAL_NAMES[area.toLowerCase()] = area;
      for (const s of subareas) {
        CANONICAL_NAMES[s.toLowerCase()] = s;
      }
    }
  }
}

// ======================== ASSUNTO_ALIASES ========================

// Mapa: variante suja do banco → nome canônico (subárea ou área)
// Valor "" = lixo, será filtrado
// Chaves DEVEM ser lowercase (a busca é case-insensitive)
const ASSUNTO_ALIASES: Record<string, string> = {
  // ── Lixo ──────────────────────────────────────────────────────────────────
  "teste": "",
  "treste": "",

  // ── Física — variantes de nomes canônicos ─────────────────────────────────
  "dinâmica e forças":               "Dinâmica",
  "equilíbrio de forças":            "Estática",
  "conservação de energia mecânica": "Conservação de Energia",
  "conservação da energia mecânica": "Conservação de Energia",
  "energia mecânica":                "Conservação de Energia",
  "energia e trabalho":              "Trabalho e Energia",
  "ondulatória":                     "Ondulatória e Acústica",
  "eletricidade":                    "Eletrodinâmica",
  "eletromagnetismo":                "Magnetismo e Eletromagnetismo",
  "termologia":                      "Termodinâmica",
  "pressão":                         "Hidrostática",
  "astronomia":                      "Gravitação Universal",
  "referencial inercial":            "Leis de Newton",
  "gravitação e teoria cinética dos gases": "Gravitação Universal",
  "gravitação":                      "Gravitação Universal",
  "associação de resistores":        "Circuitos em Série e Paralelo",
  "caráter dual da luz":             "Física Moderna",
  "dinâmica — atrito":               "Força de Atrito",
  "empuxo arquimediano":             "Hidrostática",
  "equação fundamental da ondulatória": "Ondulatória e Acústica",
  "gases":                           "Gases Ideais e Transformações",
  "ondas estacionárias":             "Ondulatória e Acústica",
  "sistemas conservativos na dinâmica": "Conservação de Energia",
  "sistemas isolados":               "Termodinâmica",
  "radioatividade":                  "Radioatividade e Decaimento",

  // ── Biologia — variantes de nomes canônicos ───────────────────────────────
  "síntese de proteínas":            "Síntese de Proteínas (Transcrição e Tradução)",
  "biotecnologia":                   "Biotecnologia e Engenharia Genética",
  "transgenia":                      "Biotecnologia e Engenharia Genética",
  "programas de saúde":              "Microbiologia e Virologia",
  "divisão celular":                 "Divisão Celular — Mitose",
  "seleção natural":                 "Seleção Natural e Adaptação",
  "herança e determinação do sexo":  "Herança Ligada ao Sexo",
  "genética de populações":          "Genética de Populações e Equilíbrio de Hardy-Weinberg",
  "genética molecular":              "Biologia Molecular",
  "antígenos, anticorpos e vacinas": "Sistema Imunológico",
  "briófitas e evolução das plantas": "Briófitas e Pteridófitas",
  "embriologia":                     "Fecundação e Desenvolvimento Embrionário",
  "imunologia":                      "Sistema Imunológico",
  "biologia celular":                "Citologia",
  "bioquímica":                      "Biologia Molecular",
  "fisiologia celular":              "Citologia",
  "microbiologia e imunologia":      "Sistema Imunológico",
  "parasitologia e saúde pública":   "Zoologia",
  "sistemática":                     "Classificação dos Seres Vivos",
  "zoologia e classificação":        "Zoologia",
  "biomas mundiais":                 "Biomas e Vegetação",
  "cnidários":                       "Poríferos e Cnidários",
  "mecanismos de especiação":        "Especiação",
  "mutações":                        "Mutações Gênicas e Cromossômicas",

  // ── Filosofia — variantes de nomes canônicos ──────────────────────────────
  "pré-socráticos":        "Pré-Socráticos e Cosmologia",
  "escola de frankfurt":   "Frankfurt e a Teoria Crítica",
  "formas de governo":     "Estado, Poder e Soberania",
  "ética":                 "Ética e Moral",
  "justiça":               "Justiça e Direito",
  "utilitarismo":          "Utilitarismo (Bentham e Mill)",
  "filosofia política":    "Filosofia Política",
  "poder e disciplina":    "Poder e Disciplina",
  "utopia":                "Utopia",
  "retórica":              "Retórica",
  "altruísmo":             "Altruísmo",
  "foucault e panoptismo": "Poder e Disciplina",
  "michel foucault":       "Poder e Disciplina",
  "francis bacon":         "Renascimento Cultural e Científico",
  "mito e filosofia":      "Pré-Socráticos e Cosmologia",
  "rousseau":              "Estado, Poder e Soberania",
  "contrato social":       "Contratualismo (Hobbes, Locke, Rousseau)",
  "empirismo":             "Empirismo (Bacon, Locke, Hume)",
  "epistemologia":         "Epistemologia e Teoria do Conhecimento",
  "filosofia clássica":    "Filosofia Antiga",
  "filosofia do tempo":    "Epistemologia e Teoria do Conhecimento",
  "platão":                "Platão e a Teoria das Ideias",
  "teoria do estado":      "Estado, Poder e Soberania",

  // ── Geografia — variantes de nomes canônicos ──────────────────────────────
  "geopolítica":               "Geopolítica Mundial",
  "climatologia":              "Climatologia e Meteorologia",
  "cartografia":               "Cartografia e Orientação",
  "agricultura":               "Agricultura Moderna e Agronegócio",
  "agronegócio":               "Agricultura Moderna e Agronegócio",
  "urbanização":               "Urbanização e Metropolização",
  "questão ambiental":         "Questões Ambientais",
  "meio ambiente":             "Questões Ambientais",
  "biomas":                    "Biomas e Vegetação",
  "geomorfologia":             "Geomorfologia e Relevo",
  "questão agrária":           "Reforma Agrária",
  "hidrografia":               "Bacias Hidrográficas",
  "urbanismo":                 "Problemas Urbanos",
  "migração":                  "Migrações Internas e Internacionais",
  "migrações":                 "Migrações Internas e Internacionais",
  "migração e memória":        "Migrações Internas e Internacionais",
  "densidade demográfica":     "Crescimento Populacional",
  "demografia":                "Crescimento Populacional",
  "habitação":                 "Problemas Urbanos",
  "rede urbana":               "Urbanização e Metropolização",
  "cidades sustentáveis":      "Desenvolvimento Sustentável",
  "comércio exterior":         "Fluxos Comerciais e Financeiros",
  "indústria e trabalho":      "Industrialização e Desindustrialização",
  "industrialização e desenvolvimento": "Industrialização e Desindustrialização",
  "transporte e logística":    "Agropecuária e Indústria",
  "transportes":               "Agropecuária e Indústria",
  "matriz energética":         "Fontes de Energia",
  "amazônia":                  "Fronteiras e Espaço Amazônico",
  "organização territorial do brasil": "Regionalização do Brasil",
  "integração regional sul-americana": "MERCOSUL e Relações Internacionais",
  "agropecuária brasileira":   "Agropecuária e Indústria",
  "produção agropecuária brasileira": "Agropecuária e Indústria",
  "brasil: agropecuária":      "Agropecuária e Indústria",
  "brasil: clima":             "Climatologia e Meteorologia",
  "brasil: domínios morfoclimáticos": "Climatologia e Meteorologia",
  "brasil: industrialização":  "Industrialização e Desindustrialização",
  "conflitos na ásia e oceania": "Geopolítica Mundial",
  "domínios morfoclimáticos":  "Climatologia e Meteorologia",
  "escala cartográfica":       "Cartografia e Orientação",
  "estrutura fundiária brasileira": "Reforma Agrária",
  "geopolítica da áfrica":     "Geopolítica Mundial",
  "globalização":              "Globalização e Economia",
  "indicadores socioeconômicos": "IDH e Indicadores de Desenvolvimento",
  "oriente médio":             "Oriente Médio e Geopolítica do Petróleo",
  "principais indicadores sociais": "IDH e Indicadores de Desenvolvimento",
  "problemas ambientais":      "Questões Ambientais",
  "solos":                     "Geomorfologia e Relevo",
  "áfrica — fronteiras e diversidade étnica": "África e Neocolonialismo Contemporâneo",
  "dinâmica demográfica e saúde": "Crescimento Populacional",
  "fenômenos geográficos e astronômicos": "Fusos Horários",
  "geopolítica e obras de engenharia": "Geopolítica Mundial",
  "blocos econômicos":         "Blocos Econômicos",
  "globalização e economia":   "Globalização e Economia",
  "geografia econômica":       "Globalização e Economia",
  "urbanização e metropolização": "Urbanização e Metropolização",
  "geopolítica ambiental":     "Questões Ambientais",
  "recursos hídricos":         "Bacias Hidrográficas",
  "recursos naturais":         "Questões Ambientais",
  "resíduos sólidos":          "Questões Ambientais",
  "geologia":                  "Tectônica de Placas",
  "estrutura interna da terra": "Tectônica de Placas",
  "movimentos da terra":       "Fusos Horários",
  "desenvolvimento humano":    "Crescimento Populacional",

  // ── História — variantes de nomes canônicos ───────────────────────────────
  "imperialismo":                  "Imperialismo Europeu (Século XIX)",
  "américa colonial":              "Colonização das Américas",
  "antiguidade oriental":          "Outras Civilizações Antigas",
  "ciclo do ouro e diamantes":     "Ciclo do Ouro e Mineração",
  "colonização e pós-colonialismo": "Resistência dos Povos Colonizados",
  "cuba no século xx":             "Conflitos da Guerra Fria (Coreia, Vietnã, Cuba)",
  "entre-guerras":                 "Entre Guerras e Totalitarismo",
  "era vargas":                    "Era Vargas e Estado Novo",
  "independências da américa latina": "Processos de Independência",
  "mineração no brasil colonial":  "Ciclo do Ouro e Mineração",
  "revoluções inglesas":           "Revolução Inglesa e Gloriosa",
  "romantismo":                    "Romantismo no Brasil",
  "unificação da itália":          "Imperialismo Europeu (Século XIX)",
  "reforma protestante":           "Reforma Protestante e Contrarreforma",
  "ditadura militar brasileira":   "Ditadura Militar (1964–1985)",
  "ditadura militar":              "Ditadura Militar (1964–1985)",
  "brasil republicano":            "Brasil República",
  "república populista":           "Democracia Populista (1945–1964)",
  "história moderna":              "Idade Moderna",
  "religião e conhecimento":       "Reforma Protestante e Contrarreforma",
  "brasil joanino":                "Período Joanino",
  "república velha e revolução de 1930": "Tenentismo e Revolução de 1930",
  "abolição da escravidão no brasil": "Abolição da Escravidão",
  "brasil império":                "Brasil Imperial",
  "primeira metade do século xx":  "Entre Guerras e Totalitarismo",
  "história econômica":            "Industrialização e Capitalismo",
  "república oligárquica":         "República Velha e Política do Café com Leite",
  "democracia no brasil":          "Redemocratização e Constituição de 1988",
  "primeira república":            "República Velha e Política do Café com Leite",
  "fascismo e nazismo":            "Entre Guerras e Totalitarismo",
  "colonização das américas":      "Conquista da América",
  "revolução científica":          "Renascimento Cultural e Científico",
  "escravidão nas américas":       "Escravidão Africana e Tráfico Negreiro",
  "história do brasil colonial":   "Brasil Colonial",
  "memória histórica":             "Brasil Contemporâneo",
  "atualidades":                   "Brasil Contemporâneo",
  "nova república":                "Nova República e Governo Collor",
  "período joanino":               "Período Joanino",
  "populismo na américa latina":   "Caudilhismo",
  "independências na américa":     "Independência Americana",
  "história contemporânea":        "Guerra Fria",
  "união europeia e integração europeia": "Fim da Guerra Fria e Dissolução da URSS",
  "revoltas e movimentos no brasil colonial": "Quilombos e Resistência",
  "história da áfrica":            "Pan-Africanismo e Movimentos de Libertação",
  "urbanização no brasil república": "Brasil República",
  "cidadania no brasil":           "Democracia e Participação Política",
  "américa portuguesa":            "Brasil Colonial",
  "roma antiga":                   "Roma Antiga",
  "história do cinema":            "Brasil Contemporâneo",
  "segundo reinado":               "Segundo Reinado",
  "partilha da África e ásia":     "Partilha da África e Ásia",
  "brasil colônia":                "Brasil Colonial",
  "descobrimentos e expansão marítima": "Expansão Marítima e Colonialismo",
  "expansão marítima":             "Expansão Marítima e Colonialismo",
  "descolonização":                "Pan-Africanismo e Movimentos de Libertação",
  "desenvolvimento econômico":     "Industrialização e Capitalismo",
  "guerra do vietnã":              "Conflitos da Guerra Fria (Coreia, Vietnã, Cuba)",
  "história antiga":               "Grécia Antiga",
  "romantismo e realismo":         "Romantismo no Brasil",

  // ── Língua Portuguesa — variantes ─────────────────────────────────────────
  "interporetação de texto":        "Interpretação de Texto",
  "interpretação textual":          "Interpretação de Texto",
  "interpretação de poema":         "Texto Lírico e Poesia",
  "gêneros textuais":               "Tipologia e Gêneros Textuais",
  "publicidade e propaganda":       "Texto Publicitário e Midiático",
  "argumentação":                   "Texto Argumentativo",
  "coesão textual":                 "Coerência e Coesão",
  "coesão referencial":             "Coerência e Coesão",
  "coesão e coerência":             "Coerência e Coesão",
  "reportagem":                     "Texto Informativo e Jornalístico",
  "meios de comunicação":           "Texto Publicitário e Midiático",
  "campanha social":                "Texto Publicitário e Midiático",
  "oralidade":                      "Língua Oral e Escrita",
  "poesia":                         "Texto Lírico e Poesia",
  "recursos estilísticos":          "Figuras de Linguagem",
  "recursos expressivos e uso de aspas": "Figuras de Linguagem",
  "tipologia textual":              "Tipologia e Gêneros Textuais",
  "concordância nominal":           "Concordância Nominal e Verbal",
  "concordância verbal":            "Concordância Nominal e Verbal",
  "concordância":                   "Concordância Nominal e Verbal",
  "regência":                       "Regência Nominal e Verbal",
  "morfologia":                     "Morfologia e Classes de Palavras",
  "morfossintaxe":                  "Morfologia e Classes de Palavras",
  "verbos":                         "Verbos e Conjugação",
  "verbos e conjugación":           "Verbos e Conjugação",
  "acentuação":                     "Ortografia e Acentuação",
  "léxico":                         "Semântica",
  "literatura":                     "Literatura Brasileira",
  "arcadismo":                      "Arcadismo Brasileiro",
  "regionalismo modernista":        "Modernismo — Segunda Fase",
  "teoria literária":               "Literatura Brasileira",
  "leitura de gráfico":             "Tabelas e Gráficos Estatísticos",
  "vozes verbais":                  "Verbos e Conjugação",
  "estratégias argumentativas":     "Tese e Argumentação",
  "patrimônio linguístico":         "Variedades Dialetais",
  "práticas de escrita":            "Coesão e Coerência Textual",
  "norma-padrão":                   "Ortografia e Acentuação",
  "ortografia":                     "Ortografia e Acentuação",
  "análise sintática":              "Sintaxe",
  "análise sintática e semântica":  "Sintaxe",
  "morfosintaxe":                   "Morfologia e Classes de Palavras",
  "morfossintáxe":                  "Morfologia e Classes de Palavras",
  "pronomes e colocação pronominal": "Pronomes",
  "período composto":               "Período Simples e Composto",
  "período simples":                "Período Simples e Composto",
  "orações subordinadas":           "Orações Subordinadas",
  "crase":                          "Crase",
  "pontuação":                      "Pontuação",
  "figuras de sintaxe":             "Figuras de Sintaxe",
  "vícios de linguagem":            "Vícios de Linguagem",
  "formação de palavras":           "Formação de Palavras",
  "semântica e vocabulário":        "Semântica",
  "gramática e norma culta":        "Gramática",
  "norma culta":                    "Gramática",
  "redação e produção de texto":    "Dissertação-Argumentativa",
  "produção textual":               "Dissertação-Argumentativa",
  "advérbio":                       "Advérbios e Conjunções",
  "conjunção":                      "Advérbios e Conjunções",
  "análise comparativa de obras":   "Intertextualidade",
  "alguma poesia":                  "Modernismo — Primeira Fase",
  "campo geral":                    "Modernismo — Segunda Fase",
  "linguagem verbal e não verbal":  "Funções da Linguagem",
  "pressupostos e subentendidos":   "Inferência e Subentendido",

  // ── Língua Inglesa — variantes ────────────────────────────────────────────
  "gramática inglesa":              "Grammar",
  "vocabulary":                     "Vocabulary",
  "interpretação de texto em inglês": "Interpretação de Texto",

  // ── Língua Espanhola — variantes ─────────────────────────────────────────
  "gramática espanhola": "Gramática",
  "vocabulário":         "Vocabulario",

  // ── Matemática — variantes ────────────────────────────────────────────────
  "radicais":             "Radiciação",
  "divisibilidade e mmc": "MMC e MDC",
  "volume de cilindro":   "Cilindros",
  "expressões algébricas": "Expressões e Frações Algébricas",
  "equação do 2º grau":   "Equação do Segundo Grau",
  "equações exponenciais / logaritmos": "Equações Exponenciais",
  "equações e sistemas":  "Sistemas Lineares",
  "funções trigonométricas e logaritmos": "Funções Trigonométricas",
  "matrizes e determinantes": "Matrizes",
  "polinômios e números complexos": "Polinômios",
  "trigonometria, somatórios": "Trigonometria",

  // ── Química — variantes ───────────────────────────────────────────────────
  "funções orgânicas":              "Funções Orgânicas Oxigenadas",
  "funções orgânicas e massa molar": "Funções Orgânicas Oxigenadas",
  "funções inorgânicas":            "Química Geral",
  "soluções":                       "Solubilidade",
  "tabela periódica":               "Tabela Periódica e Propriedades Periódicas",
  "estrutura atômica":              "Estrutura Atômica e Modelos Atômicos",
  "cálculos químicos":              "Estequiometria",
  "ácido e base":                   "Ácidos e Bases — Conceitos",
  "ácidos e bases":                 "Ácidos e Bases — Conceitos",
  "oxirredução":                    "Oxidação e Redução (Redox)",
  "número de oxidação":             "Número de Oxidação (NOX)",
  "alcanos e nomenclatura iupac":   "Alcanos e Nomenclatura IUPAC",
  "reações inorgânicas":            "Reações Inorgânicas",
  "reações químicas":               "Reações Químicas e Balanceamento",
  "misturas":                       "Substâncias e Misturas",
  "equilíbrio ácido-base":          "Equilíbrio Químico",
  "química inorgânica":             "Química Geral",
  "corrosão":                       "Corrosão e Proteção Metálica",
  "físico-química":                 "Química Geral",
  "isótopos, isóbaros e isótonos":  "Estrutura Atômica e Modelos Atômicos",
  "ligações químicas":              "Ligações Químicas e Estrutura Molecular",
  "modelo de bohr":                 "Estrutura Atômica e Modelos Atômicos",
  "potencial padrão de redução":    "Pilhas e Células Galvânicas",
  "propriedades físicas":           "Substâncias e Misturas",
  "propriedades periódicas":        "Tabela Periódica e Propriedades Periódicas",
  "propriedades das substâncias":   "Substâncias e Misturas",
  "química ambiental":              "Química Ambiental e Sustentabilidade",
  "reações de aminas":              "Funções Orgânicas Nitrogenadas",
  "reações de ácidos carboxílicos": "Funções Orgânicas Oxigenadas",
  "ácidos, bases e ph":             "Ácidos e Bases — Conceitos",
  "transformações cíclicas":        "Máquinas Térmicas e Ciclo de Carnot",

  // ── Sociologia — variantes ────────────────────────────────────────────────
  "povos indígenas":                       "Movimentos Indígenas",
  "diversidade cultural":                  "Cultura e Diversidade",
  "correntes sociológicas":                "Clássicos da Sociologia",
  "surgimento da sociologia":              "Clássicos da Sociologia",
  "sociologia":                            "Clássicos da Sociologia",
  "marxismo":                              "Karl Marx e o Materialismo Histórico",
  "instituições sociais":                  "Clássicos da Sociologia",
  "raça e racismo":                        "Racismo e Discriminação",

  // trabalho
  "alienação do trabalho":                 "Trabalho e Alienação",
  "capitalismo":                           "Capitalismo e suas Fases",
  "capitalismo e trabalho assalariado":    "Capitalismo e suas Fases",
  "fordismo e pós-fordismo":               "Fordismo, Taylorismo e Toyotismo",
  "taylorismo":                            "Fordismo, Taylorismo e Toyotismo",
  "taylorismo e fordismo":                 "Fordismo, Taylorismo e Toyotismo",
  "divisão do trabalho":                   "Divisão do Trabalho",
  "trabalho doméstico":                    "Trabalho Doméstico",
  "relações de trabalho":                  "Trabalho na Sociedade Contemporânea",
  "trabalho e tecnologia":                 "Trabalho na Sociedade Contemporânea",
  "flexibilização do trabalho":            "Desemprego e Precarização do Trabalho",

  // raça e gênero
  "trabalho feminino":                     "Trabalho Feminino",
  "desigualdade racial":                   "Racismo e Discriminação",
  "trabalho escravo":                      "Escravidão e seus Legados",

  // estado e democracia
  "estado, poder e soberania":             "Estado e Formas de Governo",

  // ── Arte — variantes ──────────────────────────────────────────────────────
  "artes indígenas":      "Arte Indígena",
  "arte contemporânea":   "Arte Contemporânea",

  // ── Educação Física — variantes ───────────────────────────────────────────
  "lutas":           "Lutas e Artes Marciais",
  "esporte adaptado": "Esportes Paralímpicos",

  // ── Aritmética ────────────────────────────────────────────────────────────
  "divisores": "Múltiplos e Divisores",
  "múltiplos": "Múltiplos e Divisores",
  "divisores e múltiplos": "Múltiplos e Divisores",
  "divisibilidade e múltiplos": "Múltiplos e Divisores",
  "mmc": "MMC e MDC",
  "mdc": "MMC e MDC",
  "mmc e mdc": "MMC e MDC",
  "razão": "Razões e Proporções",
  "proporção": "Razões e Proporções",
  "proporcionalidade": "Razões e Proporções",
  "razão e proporção": "Razões e Proporções",
  "regra de três simples": "Regra de Três",
  "regra de três composta": "Regra de Três",
  "regra de 3": "Regra de Três",
  "potências": "Potenciação",
  "potência": "Potenciação",
  "raízes": "Radiciação",
  "raiz quadrada": "Radiciação",
  "radiciação e potenciação": "Radiciação",
  "pa": "Progressão Aritmética",
  "pg": "Progressão Geométrica",
  "progressões": "Progressão Aritmética",
  "sequências": "Progressão Aritmética",
  "sequências numéricas": "Progressão Aritmética",
  "média aritmética": "Médias",
  "média ponderada": "Médias",
  "médias aritméticas": "Médias",
  "grandezas e medidas": "Grandezas Proporcionais",
  "grandezas inversamente proporcionais": "Grandezas Proporcionais",
  "juros": "Juros Simples",
  "juros e descontos": "Juros Simples",
  "sistema métrico": "Sistema de Numeração e Métrico Decimal",
  "notação científica": "Notação Científica",
  "frações": "Expressões e Frações Algébricas",
  "frações e decimais": "Expressões e Frações Algébricas",
  "operações": "Problemas sobre as 4 Operações",

  // ── Conjuntos Numéricos ───────────────────────────────────────────────────
  "números": "Conjuntos Numéricos",
  "números inteiros": "Conjuntos Numéricos",
  "números naturais": "Conjuntos Numéricos",
  "números racionais": "Conjuntos Numéricos",
  "números reais": "Conjuntos Numéricos",
  "teoria dos números": "Conjuntos Numéricos",
  "conjuntos numéricos": "Conjuntos Numéricos",

  // ── Álgebra ───────────────────────────────────────────────────────────────
  "equações": "Equação do Primeiro Grau",
  "equação linear": "Equação do Primeiro Grau",
  "equação de 1o grau": "Equação do Primeiro Grau",
  "equação de 1º grau": "Equação do Primeiro Grau",
  "equações do 1º grau": "Equação do Primeiro Grau",
  "equações de 1o grau": "Equação do Primeiro Grau",
  "equação de 2o grau": "Equação do Segundo Grau",
  "equação de 2º grau": "Equação do Segundo Grau",
  "equações do 2º grau": "Equação do Segundo Grau",
  "equação quadrática": "Equação do Segundo Grau",
  "equações quadráticas": "Equação do Segundo Grau",
  "equação exponencial": "Equações Exponenciais",
  "inequações": "Inequação do Primeiro Grau",
  "inequação linear": "Inequação do Primeiro Grau",
  "inequação do 2o grau": "Inequação do Segundo Grau",
  "inequações modulares": "Inequações Modulares",
  "sistemas de equações": "Sistemas Lineares",
  "sistema linear": "Sistemas Lineares",
  "sistemas": "Sistemas Lineares",
  "álgebra linear": "Sistemas Lineares",
  "fatoração": "Fatoração e Produtos Notáveis",
  "produtos notáveis": "Fatoração e Produtos Notáveis",
  "fatoração algébrica": "Fatoração e Produtos Notáveis",
  "módulo": "Equações Modulares",
  "valor absoluto": "Equações Modulares",
  "integral definida": "Integrais",

  // ── Funções ───────────────────────────────────────────────────────────────
  "função": "Funções",
  "funções afins": "Função Afim",
  "função linear": "Função Afim",
  "função do 1o grau": "Função Afim",
  "função do 1º grau": "Função Afim",
  "funções quadráticas": "Função Quadrática",
  "função do 2o grau": "Função Quadrática",
  "função do 2º grau": "Função Quadrática",
  "parábola (função)": "Função Quadrática",
  "exponencial": "Função Exponencial",
  "função exponencial e logarítmica": "Função Exponencial",
  "logaritmo": "Função Logarítmica",
  "logaritmos": "Função Logarítmica",
  "funções logarítmicas": "Função Logarítmica",
  "função trigonométrica": "Funções Trigonométricas",
  "função racional": "Funções",
  "composição de funções": "Função Composta",
  "funções e composição": "Função Composta",
  "domínio e imagem": "Domínio, Imagem e Contradomínio",
  "domínio e contradomínio": "Domínio, Imagem e Contradomínio",
  "domínio": "Domínio, Imagem e Contradomínio",
  "imagem de funções": "Domínio, Imagem e Contradomínio",
  "gráficos": "Gráficos e Transformações",
  "gráficos de funções": "Gráficos e Transformações",
  "transformações de gráficos": "Gráficos e Transformações",
  "interpretação de gráficos": "Tabelas e Gráficos Estatísticos",
  "eometria plana": "Geometria Plana",

  // ── Análise Combinatória ──────────────────────────────────────────────────
  "combinatoria": "Análise Combinatória",
  "combinatória": "Análise Combinatória",
  "combinação": "Combinações",
  "permutação": "Permutações Simples",
  "arranjo": "Arranjos",
  "contagem": "Princípio Fundamental da Contagem",
  "princípio da contagem": "Princípio Fundamental da Contagem",
  "princípio multiplicativo": "Princípio Fundamental da Contagem",
  "binômio": "Binômio de Newton",

  // ── Estatística ───────────────────────────────────────────────────────────
  "tabelas e gráficos": "Tabelas e Gráficos Estatísticos",
  "gráficos estatísticos": "Tabelas e Gráficos Estatísticos",
  "moda": "Medidas de Tendência Central",
  "mediana": "Medidas de Tendência Central",
  "média, moda e mediana": "Medidas de Tendência Central",
  "medidas estatísticas": "Medidas de Tendência Central",
  "desvio padrão": "Medidas de Dispersão",
  "variância": "Medidas de Dispersão",
  "frequência": "Distribuição de Frequências",
  "quartil": "Medidas de Posição",
  "percentil": "Medidas de Posição",

  // ── Probabilidade ─────────────────────────────────────────────────────────
  "espaço amostral": "Espaço Amostral e Eventos",
  "binomial": "Distribuição Binomial",

  // ── Geometria Plana ───────────────────────────────────────────────────────
  "geometria": "Geometria Plana",
  "geometria plana e otimização": "Geometria Plana",
  "semelhança": "Semelhança de Triângulos",
  "congruência": "Congruência de Triângulos",
  "pitágoras": "Teorema de Pitágoras",
  "tales": "Teorema de Tales",
  "área": "Área das Figuras Planas",
  "áreas e perímetros": "Área das Figuras Planas",
  "perímetro": "Área das Figuras Planas",
  "círculo": "Circunferência e Círculo",

  // ── Geometria Espacial ────────────────────────────────────────────────────
  "volume": "Geometria Espacial",
  "volumes": "Geometria Espacial",
  "sólidos geométricos": "Geometria Espacial",
  "geometria espacial e volume": "Geometria Espacial",
  "prisma": "Prismas",
  "pirâmide": "Pirâmides",
  "cilindro": "Cilindros",
  "cone": "Cones",

  // ── Trigonometria ─────────────────────────────────────────────────────────
  "trigonometria espacial": "Trigonometria",
  "seno e cosseno": "Razões Trigonométricas no Triângulo Retângulo",
  "seno, cosseno e tangente": "Razões Trigonométricas no Triângulo Retângulo",
  "razões trigonométricas": "Razões Trigonométricas no Triângulo Retângulo",
  "arcos": "Arcos e Medidas de Arcos",
  "radianos": "Arcos e Medidas de Arcos",

  // ── Matemática — novos aliases ────────────────────────────────────────────
  "area do setor":                         "Circunferência e Círculo",
  "arranjo simples e com repetição":       "Arranjos",
  "combinação simples":                    "Combinações",
  "comprimento da circunferência":         "Circunferência e Círculo",
  "conceitos iniciais de funções":         "Funções",
  "conceitos primitivos e geometria de posição": "Geometria Plana",
  "função definida por sentenças":         "Funções Definidas por Várias Sentenças",
  "função seno":                           "Funções Trigonométricas",
  "gráficos no plano cartesiano":          "Geometria Analítica",
  "leitura de gráficos":                   "Tabelas e Gráficos Estatísticos",
  "medidas de tempo":                      "Sistema de Numeração e Métrico Decimal",
  "naturais":                              "Conjuntos Numéricos",
  "pontos notáveis em um triângulo":       "Pontos Notáveis do Triângulo",
  "racionais":                             "Conjuntos Numéricos",
  "transformação de unidades de medida":   "Sistema de Numeração e Métrico Decimal",
  "área de polígonos":                     "Área das Figuras Planas",
  "áreas de quadriláteros":               "Área das Figuras Planas",
  "volume do prisma":                      "Prismas",

  // ── Aliases de assuntos que não existem no JSON mas aparecem no banco ────
  "cônicas": "Geometria Analítica",
  "cónicas": "Geometria Analítica",
  "secções cônicas": "Geometria Analítica",
  "inequação racional": "Inequações Produto e Quociente",
  "inequações racionais": "Inequações Produto e Quociente",
  "números decimais": "Expressões e Frações Algébricas",
  "números decimais e frações": "Expressões e Frações Algébricas",
  "problemas com restrições": "Sistemas Lineares",
  "teoria dos conjuntos": "Conjuntos",
  "conjuntos e subconjuntos": "Conjuntos",

  // ── Lógica Matemática ─────────────────────────────────────────────────────
  "lógica": "Lógica Matemática",
  "lógica proposicional": "Proposições e Conectivos",
  "proposições": "Proposições e Conectivos",
  "conectivos lógicos": "Proposições e Conectivos",
  "tabela verdade": "Tabela-Verdade",
  "diagramas de venn": "Diagramas Lógicos",
  "raciocínio lógico": "Lógica Matemática",

  // ── Análise Matemática ────────────────────────────────────────────────────
  "cálculo": "Análise Matemática",
  "derivada": "Derivadas",
  "otimização": "Análise Matemática",
  "máximos e mínimos": "Derivadas",
  "taxa de variação": "Derivadas",
  "integral": "Integrais",
};

/**
 * Limpa aspas e apóstrofos das pontas.
 */
function stripQuotes(s: string): string {
  return s.replace(/^["']+|["']+$/g, "").trim();
}

/**
 * Normaliza um assunto:
 * 1. Tenta alias explícito (case-insensitive)
 * 2. Tenta casar com nome conhecido (área/subárea) case-insensitive
 * 3. Se nada bater, retorna como veio (trimmed)
 */
export function normalizeAssunto(raw: string): string {
  const trimmed = stripQuotes(raw.trim());
  if (!trimmed) return "";
  const lower = trimmed.toLowerCase();

  // Alias explícito
  const alias = ASSUNTO_ALIASES[lower];
  if (alias !== undefined) return alias; // "" = lixo, string = canônico

  // Nome conhecido com grafia diferente (ex: "geometria plana" → "Geometria Plana")
  const canonical = CANONICAL_NAMES[lower];
  if (canonical) return canonical;

  // Remove parêntesis e tenta de novo: "Otimização (Mínimos)" → "Otimização"
  const semParen = trimmed.replace(/\s*\([^)]*\)/g, "").trim();
  if (semParen && semParen !== trimmed) {
    const r = normalizeAssunto(semParen);
    if (r) return r;
  }

  // Valor composto com separadores — tenta cada parte individualmente
  if (/,| \/ | e /.test(trimmed)) {
    for (const part of trimmed.split(/,| \/ | e /)) {
      const p = stripQuotes(part.trim());
      if (!p) continue;
      const pLower = p.toLowerCase();
      const partAlias = ASSUNTO_ALIASES[pLower];
      if (partAlias !== undefined && partAlias !== "") return partAlias;
      const partCanonical = CANONICAL_NAMES[pLower];
      if (partCanonical) return partCanonical;
    }
  }

  // Desconhecido — retorna como está
  return trimmed;
}

// ======================== Disciplinas ========================

// Disciplinas conhecidas (chave lowercase → canônico)
const DISCIPLINAS_CANONICAS: Record<string, string> = {
  "matemática": "Matemática",
  "física": "Física",
  "química": "Química",
  "biologia": "Biologia",
  "língua portuguesa": "Língua Portuguesa",
  "história": "História",
  "geografia": "Geografia",
  "filosofia": "Filosofia",
  "sociologia": "Sociologia",
  "língua inglesa": "Língua Inglesa",
  "língua espanhola": "Língua Espanhola",
  "língua francesa": "Língua Francesa",
  "educação física": "Educação Física",
  "artes": "Artes",
  "ciências": "Ciências",
  "redação": "Redação",
  "informática": "Informática",
  "programação": "Programação",
};

// Aliases explícitos de disciplinas sujas (chave lowercase → canônico ou "" pra lixo)
const DISCIPLINA_ALIASES: Record<string, string> = {
  // Matemática — variantes sujas
  "matemáticas": "Matemática",
  "matemáticafunções afins": "Matemática",
  // Língua Portuguesa — formas curtas
  "português": "Língua Portuguesa",
  "portugues": "Língua Portuguesa",
  "lingua portuguesa": "Língua Portuguesa",
  "literatura": "Língua Portuguesa",
  "literatura brasileira": "Língua Portuguesa",
  // Língua Inglesa — formas curtas
  "inglês": "Língua Inglesa",
  "ingles": "Língua Inglesa",
  "lingua inglesa": "Língua Inglesa",
  // Língua Espanhola — formas curtas
  "espanhol": "Língua Espanhola",
  "lingua espanhola": "Língua Espanhola",
  // Língua Francesa — formas curtas
  "francês": "Língua Francesa",
  "frances": "Língua Francesa",
  "lingua francesa": "Língua Francesa",
  // Artes — variante sem 's'
  "arte": "Artes",
  // Lixo
  "testetes": "",
};

/**
 * Normaliza uma disciplina:
 * 1. Alias explícito (case-insensitive)
 * 2. Nome canônico conhecido (case-insensitive)
 * 3. Se nada bater, retorna como veio
 */
export function normalizeDisciplina(raw: string): string {
  const trimmed = stripQuotes(raw.trim());
  if (!trimmed) return "";
  const lower = trimmed.toLowerCase();

  const alias = DISCIPLINA_ALIASES[lower];
  if (alias !== undefined) return alias;

  const canonical = DISCIPLINAS_CANONICAS[lower];
  if (canonical) return canonical;

  return trimmed;
}

// ======================== Assuntos (autocomplete) ========================

// Todas as subáreas + nomes de áreas de TODAS as disciplinas e níveis, para autocomplete
export const ASSUNTOS_CANONICOS: string[] = (() => {
  const set = new Set<string>();
  for (const nivelMap of Object.values(DISCIPLINAS_JSON)) {
    for (const areasMap of Object.values(nivelMap)) {
      if (!areasMap) continue;
      for (const [area, { subareas }] of Object.entries(areasMap)) {
        set.add(area);
        for (const s of subareas) set.add(s);
      }
    }
  }
  return [...set].sort((a, b) => a.localeCompare(b, "pt-BR"));
})();

/**
 * Dado uma lista de assuntos (já normalizados), agrupa por área.
 * Reconhece tanto nomes de subáreas quanto nomes de áreas.
 * Assuntos que não pertencem a nenhuma área vão para "Outros".
 *
 * @param assuntos   Lista de assuntos a agrupar
 * @param disciplina Disciplina opcional para usar o mapa correto
 * @param nivel      Nível opcional para filtrar o mapa
 */
export function groupAssuntosByArea(
  assuntos: string[],
  disciplina?: string | null,
  nivel?: string | null
): { area: string; assuntos: string[] }[] {
  const specificMap = getAreasMapPorDisciplina(disciplina, nivel);

  // Se disciplina não informada ou não encontrada: mescla todos os níveis por disciplina para
  // evitar que áreas com o mesmo nome (ex: "Aritmética" em fundamental e médio) se cancelem
  const allMaps: AreasMap[] = specificMap
    ? [specificMap]
    : Object.values(DISCIPLINAS_JSON).map((nivelMap) => {
        const merged: AreasMap = {};
        for (const m of Object.values(nivelMap)) {
          if (!m) continue;
          for (const [area, { subareas }] of Object.entries(m as AreasMap)) {
            if (merged[area]) {
              const existing = new Set(merged[area].subareas);
              for (const s of subareas) existing.add(s);
              merged[area] = { subareas: [...existing] };
            } else {
              merged[area] = { subareas: [...subareas] };
            }
          }
        }
        return merged;
      });

  const assuntoSet = new Set(assuntos);
  const used = new Set<string>();
  const groups: { area: string; assuntos: string[] }[] = [];

  for (const map of allMaps) {
    for (const [area, { subareas }] of Object.entries(map)) {
      if (used.has(area)) continue; // já processada por outro mapa

      const matched: string[] = [];

      if (assuntoSet.has(area)) matched.push(area);
      for (const s of subareas) {
        if (assuntoSet.has(s)) matched.push(s);
      }

      if (matched.length > 0) {
        groups.push({ area, assuntos: matched });
        matched.forEach((s) => used.add(s));
        used.add(area);
      }
    }
  }

  // Assuntos que não bateram com nenhuma área conhecida
  const remaining = assuntos.filter((s) => !used.has(s));
  if (remaining.length > 0) {
    groups.push({ area: "Outros", assuntos: remaining.sort() });
  }

  return groups;
}
