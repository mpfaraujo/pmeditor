...
export default function MontarProvaPage() {
  ...
  const initialLayout: ColumnLayout = useMemo(() => {
...
const renderQuestion = (question: QuestionData | undefined, globalIndex: number) => {
  ...
  useEffect(() => {
    ...
    const firstPageTop = measureFirstPageRef.current.getBoundingClientRect().top;
    const firstQuestoesTop = measureFirstQuestoesRef.current.getBoundingClientRect().top;
    const capFirst = Math.max(0, PAGE_HEIGHT - (firstQuestoesTop - firstPageTop) - SAFETY_PX);

    const otherPageTop = measureOtherPageRef.current.getBoundingClientRect().top;
    const otherQuestoesTop = measureOtherQuestoesRef.current.getBoundingClientRect().top;
    const capOther = Math.max(0, PAGE_HEIGHT - (otherQuestoesTop - otherPageTop) - SAFETY_PX);

    const itemEls = Array.from(
      measureItemsRef.current.querySelectorAll(".questao-item-wrapper")
    ) as HTMLDivElement[];

    // inclui margens do wrapper (cm -> px já convertido pelo computedStyle)
    ...
    let pageIndex = 0;
    const pageCap = () => (pageIndex === 0 ? capFirst : capOther);
  ...
  return (
    <>
      ...
      <div className="measure-layer absolute -z-10 invisible pointer-events-none">
        ...
        <div className="prova-page mx-auto bg-white" ref={measureFirstPageRef}>
          ...
          <div className="questoes-container" ref={measureFirstQuestoesRef}>
        ...
        <div className="prova-page mx-auto bg-white" ref={measureOtherPageRef}>
          <div className="questoes-container" ref={measureOtherQuestoesRef}>
            <div className="coluna coluna-1" />
            <div className="coluna coluna-2" />
          </div>
        </div>
        ...
        <div className="prova-page mx-auto bg-white">
          <div className="questoes-container">
            <div className="coluna coluna-1" ref={measureItemsRef}>
